...
var Uploader = Class.extend({
    ...
    showProgressbar: function() {
        ...
        message.setMessage('<div style="display:block;" id="progressbar"></div>');
        message.show();
...
var notif = {
    ...
    animation: false,
    init: function() {
        if(!$('#badge_notif').hasClass('invisible')) {
            this.start();
        }
    },
    start: function() {
    ...
    toggleBadge: function(nbr) {

        if(this.animation) {
            if(nbr%2) {
                $('#badge_notif').addClass('invisible');
                $('#icon_notif').removeClass('invisible');
            } else {
                $('#icon_notif').addClass('invisible');
                $('#badge_notif').removeClass('invisible');
            }

            this.timer_id = setTimeout(function() {
                this.toggleBadge(nbr+1);
            }.bind(this), 3000);

        } else {
            $('#icon_notif').removeClass('invisible red');
            $('#badge_notif').addClass('invisible');
        }
    }
}
